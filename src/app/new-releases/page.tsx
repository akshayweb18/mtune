...
export default function NewReleasesPage() {
  ...
  const { data: newReleasesData, isLoading } = useQuery({
    queryKey: ['new-releases'],
    queryFn: () => saavnApi.searchSongs('new', 1, 30),
  });

  const songs: Song[] = newReleasesData?.results || [];
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-full bg-[#05050f]">
        <div className="w-12 h-12 border-4 border-pink-500/30 border-t-pink-500 rounded-full animate-spin shadow-[0_0_15px_rgba(236,72,153,0.5)]" />
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#05050f] pb-[120px]">
      ...
      <div className="relative w-full h-[40vh] min-h-[300px] flex items-end p-6 md:p-10 z-10 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-pink-600/30 via-rose-600/10 to-[#05050f] z-0"></div>
        <div className="absolute inset-0 bg-gradient-to-t from-[#05050f] to-transparent z-0"></div>
        <div className="absolute top-1/2 left-1/4 w-96 h-96 bg-pink-500/20 blur-[60px] rounded-full opacity-60 z-0"></div>
        
        <div className="relative z-10 flex flex-col md:flex-row items-end gap-6 w-full">
          <div className="w-32 h-32 md:w-48 md:h-48 rounded-2xl bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center shadow-[0_0_40px_rgba(236,72,153,0.5)] shrink-0 border border-white/20">
             <Sparkles className="w-16 h-16 md:w-24 md:h-24 text-white drop-shadow-md" />
          </div>
          <div className="flex flex-col flex-1 pb-2">
            <span className="text-white/80 font-bold uppercase tracking-widest text-xs md:text-sm mb-2">Fresh Drops</span>
            <h1 className="text-5xl md:text-7xl font-extrabold text-white drop-shadow-lg tracking-tight mb-4">New Releases</h1>
            <p className="text-white/60 text-sm md:text-base font-medium">The latest music right now.</p>
          </div>
        </div>
        ...
        <div className="absolute -bottom-8 right-8 z-20">
           <button 
             ...
             className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-pink-400 to-rose-500 flex items-center justify-center text-white shadow-[0_0_30px_rgba(236,72,153,0.6)] hover:scale-105 active:scale-95 transition-all"
           >
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center px-4">
                  <h4 className={cn("text-[14px] md:text-[15px] font-bold truncate transition-colors", isActive ? "text-pink-400 drop-shadow-[0_0_8px_rgba(244,114,182,0.5)]" : "text-white group-hover:text-pink-300")}>
                    {song.name}
                  </h4>
